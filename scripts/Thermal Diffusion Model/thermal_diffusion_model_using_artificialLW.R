...
nt <- (1/dt)*7.5*365.   # Number of time steps
...
#plug Taylor Glacier air temperature in here as placeholder
# tried the ice surface temperature product, but the fit was way worse
outgoing_longwave_radiation <- COHM |> 
  mutate(surftemp_K = (surftemp_degc + 273.15)) |> 
  select(date_time, surftemp_K) |> 
  mutate(surftemp_K = if_else(date_time > "2023-01-05 01:45:00", surftemp_K * 1.04, surftemp_K)) |> 
  mutate(lwradout2_wm2 = (epsilon*sigma*(surftemp_K^4)))
...
incoming_longwave_radiation_initial <- COHM |> 
  ...
  mutate(lwradin2_wm2 = lwradin2_wm2)
...
new_timestamps <- seq.POSIXt(from = last_timestamp + 15*60, 
                             to = as.POSIXct("2025-12-31 23:45:00"), 
                             by = "15 min")
...
#using air temperature from Bonney Met
artificial_lw_in <- BOYM |> 
...
incoming_longwave_radiation <- artificial_lw_in |> 
  mutate(date = as.Date(date_time)) |> 
  left_join(cloud_cover_df, by = "date") |> 
  select(-date) |> # Remove the helper date column
  mutate(lwradin2_wm2 = (0.765 + 0.22*cloud_cover^3)*sigma*(airtemp_3m_K)^4)
...
time_series <- tibble(
  ...
  albedo = 0.20 + ((albedo_interp)*0.90),  # albedo, unitless (lower albedo value from measured BOYM data)
...
series <- time_series |> 
  pivot_longer(cols = c(T_air, SW_in, LWR_in, LWR_out, pressure, albedo, relative_humidity, wind, delta_T), names_to = "variable", values_to = "data")
...
for (t_idx in 1:nrow(time_series)) {
  ...
  #  Ql = rho_air*(xLatent)*Ce*(ea - es0)*wind
  #  }
  ...
  # account for ablation
  #Qnet_lw_sw <- SW_abs + (LW_net - (k * (prevT[1] - T_air) / dx))
...
write_csv(results, "data/thermal diffusion model data/model_outputs/GEE_output_corrected_20250220.csv")
