...
alllakes <- read_csv("data/sediment abundance data/sediment_abundances_20250204.csv") |> 
  mutate(date = ymd(date), 
         mean_sed = sediment*100, 
         year = year(date), 
         month = month(date))
...
summary_lakes <- alllakes |> 
  ...
  summarize(
    mean = mean(mean_coverage), 
    min = min(mean_coverage), 
    max = max(mean_coverage), 
    median = median(mean_coverage)
  ) |> 
  pivot_longer(cols = c(mean, min, max, median), names_to = "stats", values_to = "value")

#plot summary statistics
ggplot(summary_lakes, aes(x = value, fill = lake)) + 
  geom_bar() + 
