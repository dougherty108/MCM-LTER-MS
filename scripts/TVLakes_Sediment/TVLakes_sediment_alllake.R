...
files <- list.files(path = "~/Google Drive/My Drive/EarthEngine", pattern = ".tif")
...
output_tosave = output |> 
  pivot_longer(cols = c(eastlobe, hoare, fryxell, westlobe), names_to = "lake", values_to = "sediment") |> 
  mutate(date = ymd(date))

write_csv(output_tosave, "data/sediment abundance data/SENTINEL_sediment_abundances_20250217.csv")
